        ...
        if st.button("📅 Uložit kolo a pokračovat", key=f"save_continue_{rnd_idx}"):
            if not valid_round:
                ...
                if rnd_idx < st.session_state.rounds:
                    ...
                    # --- AUTOMATICKÉ ULOŽENÍ A NAČTENÍ PO POSLEDNÍM KOLE ---
                    # 1. Export dat turnaje
                    tournament_json = export_tournament_data()
                    
                    # 2. Načtení dat turnaje ze stringu JSON
                    import_tournament_data(tournament_json)
                    
                    st.info("Automatické uložení dokončeného turnaje provedeno. Zobrazuji konečné výsledky.")
                    # 3. Vynucení RERUN pro zobrazení výsledné sekce
                    st.rerun()
                    # --------------------------------------------------------
